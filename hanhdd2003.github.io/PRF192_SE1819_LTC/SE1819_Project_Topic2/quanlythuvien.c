...
void display_borrowers(struct library *lib) 
{
    ...
    for (i = 0; i < lib->num_borrowers; i++) {
        if (lib->borrowers[i].num_books == 0) {
            continue; // b? qua ngu?i mu?n không còn sách nào trong danh sách mu?n
        }
...
void return_book(struct library *lib, int borrower_id, char book_title[],int so_sach) 
{
    int flag ;
    for (i = 0; i < lib->num_borrowers; i++)
    {
        if (lib->borrowers[i].id == borrower_id)
        {
            for (j = 0; j < lib->borrowers[i].num_books; j++)
            {
                ...
                {
                    flag = 1;
                    lib->borrowers[i].num_books-=so_sach;
                    lib->books[j].quantity+=so_sach;
                    strcpy(lib->borrowers[i].book[j], "");
                    printf("%d book '%s' returned successfully by borrower %d.\n",so_sach, book_title, borrower_id);
                    break;
                }
            }
            if (flag == 0)
